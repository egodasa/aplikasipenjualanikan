...
CREATE TABLE `daftar_produk` (`Id_Produk` int(11), `Nama_Produk` varchar(50), `Stok` int(11), `Id_Sat_Produk` int(11), `Satuan` varchar(30));
...
CREATE TABLE `laporan_detail_pembelian` (`id_produk` int(11), `Id_Dpembelian` int(11), `id_pemasok` int(11), `id_pembelian` varchar(50), `Nama_Produk` varchar(50), `jumlah` int(11), `harga_satuan` decimal(10,2), `total_harga` decimal(10,2), `Satuan` varchar(30), `Nama_Pemasok` varchar(100));


DROP VIEW IF EXISTS `laporan_detail_transaksi`;
CREATE TABLE `laporan_detail_transaksi` (`Id_Transaksi` varchar(50), `Id_Produk` int(11), `Nama_Produk` varchar(50), `Jumlah` int(11), `Satuan` varchar(30));
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `laporan_detail_transaksi` AS select `a`.`id_transaksi` AS `Id_Transaksi`,`a`.`id_produk` AS `Id_Produk`,`b`.`Nama_Produk` AS `Nama_Produk`,`a`.`jumlah` AS `Jumlah`,`b`.`Satuan` AS `Satuan` from (`tbl_detail_transaksi` `a` join `daftar_produk` `b` on((`a`.`id_produk` = `b`.`Id_Produk`)));
